    ...
    <v-row>
        <v-slide-group show-arrows class="sm-3" v-model="model">
            <v-slide-item class="sm-3" v-for="(item, name) in items" :key="name" v-slot:default="{ toggle }">
                <v-card align="center" elevation="6" class="ma-3 d-flex flex-no-wrap justify-space-between" :height="size" :width="size" @click="toggle" shaped>
        ...
        <v-expand-transition>
            <v-row justify="center">
                <singleProductCard class="sm-3" v-if="model != null" :title="name" :description="description" :image="link" :price="price" />
            </v-row>
        </v-expand-transition>
    </v-row>
</v-col>
